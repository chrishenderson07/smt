import { getTournamets } from '@/api/services/entities/tournament';
...
function groupTournamentsByMonth(tournaments: Tournament[]): { month: string, tournaments: Tournament[] }[] {
  const groupedTournaments: { [key: string]: Tournament[] } = {};

  tournaments.forEach(tournament => {
    let startMonth = new Date(tournament.startDate).getMonth();
    let endMonth = tournament.endDate ? new Date(tournament.endDate).getMonth() : startMonth;
  ...
  return Object.entries(groupedTournaments).map(([month, tournaments]) => ({ month, tournaments }));
}
      ...
      <div className='wide-wrapping space-y-12'>
        {
          tournamentByMonth.map(({ month, tournaments }) => (
            <div key={month} className='space-y-6 laptop:flex laptop:space-x-3'>
              <div className='w-[10%]'>
                <Button className='capitalize'>{month}</Button>
              </div>
              <div className='grow space-y-6'>
                {
                  tournaments.map(tournament => <TournamentCalendar key={tournament.id} data={tournament}/>)
                }
              </div>
            </div>
          ))
